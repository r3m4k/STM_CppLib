...
public:
    ...
    /**
     * @brief Устанавливает начальное значение отфильтрованных данных.
     * 
     * @param init_value Константная ссылка на объект TriaxialData, содержащий трёхосные данные
     *                   (например, акселерометра, гироскопа или магнитометра), которые станут
     *                   начальным отфильтрованным состоянием.
     * 
     * @note    Данный метод позволяет задать начальное состояние фильтра, 
     *          присваивая внутреннему полю filtered_value переданное 
     *          значение init_value. Используется для инициализации фильтра
     *          перед выполнением основных шагов фильтрации.
     */
    void set_initial_value(const TriaxialData& init_value){
        filtered_value = init_value;
    }
